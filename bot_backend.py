...
try:
    ...
    print("ERROR: intent_classifier.pkl not found!")
    print("Please make sure the file is in the same directory as bot_backend.py")
    exit(1)  # Exit if critical file is missing

# Initialize embeddings using LOCAL Hugging Face model
try:
    ...
    # )
            ...
            # Update this section in bot_backend.py
    embeddings = HuggingFaceEmbeddings(
        model_name="sentence-transformers/all-MiniLM-L6-v2",
        cache_folder="/tmp"  # Use temporary directory for GCP
    )
...
try:
    ...
    print("Please make sure the 'faiss_index' directory exists and is valid")
    exit(1)  # Exit if critical file is missing

# Cloud LLM (Groq) - Explicitly pass API key
try:
    llm = ChatGroq(
...
try:
    ...
    print("✅ CrossEncoder re-ranker loaded successfully")
except Exception as e:
...
"""
        ...
        traceback.print_exc()
        print("-----------------------")
        return f"[Backend Error] An error occurred. Check the server console for details."
