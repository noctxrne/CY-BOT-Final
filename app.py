...
# Database MUST be inside /tmp on cloud platforms
DATABASE = os.path.join(tempfile.gettempdir(), "chat_sessions.db")
...
def get_db_connection():
    """Return SQLite connection at runtime-safe temporary path."""
...
def init_db():
    ...
    if not os.path.exists(DATABASE):
        with closing(get_db_connection()) as db:
            ...
            db.commit()
        print("Database created at:", DATABASE)
    else:
...
def switch_chat():
    ...
    with closing(get_db_connection()) as db:
        ...
        if exists:
            session["session_id"] = session_id
            history = db.execute(
                "SELECT user_message, bot_message FROM messages WHERE session_id = ? ORDER BY timestamp",
                (session_id,)
            ).fetchall()
...
def remove_pdf():
    ...
    if not pdf_id:
        return jsonify({"success": False, "error": "PDF ID missing"})
